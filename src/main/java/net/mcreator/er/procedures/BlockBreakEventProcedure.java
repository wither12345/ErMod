...
public class BlockBreakEventProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity) {
		...
		if (breaking_Item.getItem() == ErModItems.PYRO_HOE.get()) {
			...
			if (blockstate.is(BlockTags.create(new ResourceLocation("er:electro_block")))) {
				...
				for (ItemStack stack : drops) {
					...
					if (level.getRecipeManager().getRecipeFor(RecipeType.SMELTING, new SimpleContainer(stack), level).isPresent()) {
						item_spawn = (level.getRecipeManager().getRecipeFor(RecipeType.SMELTING, new SimpleContainer(stack), level).map(recipe -> recipe.getResultItem(level.registryAccess()).copy()).orElse(ItemStack.EMPTY));
