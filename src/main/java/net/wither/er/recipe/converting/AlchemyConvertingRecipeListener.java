...
public class AlchemyConvertingRecipeListener extends SimpleJsonResourceReloadListener {
    ...
    private static void read(ResourceLocation location, JsonObject object, RegistryAccess access){
        Item mainMaterial = BuiltInRegistries.ITEM.get(ResourceLocation.parse(object.get("item").getAsString()));
        ...
        for(JsonElement itemElement : itemsArray){
            if(itemElement.isJsonObject()){
                try {
                    ...
                    Optional<ItemStack> optionalItemStack = ItemStack.parse(access, tag);
                ...
                if (s.charAt(0) != '#') {
                    Item itemGet = BuiltInRegistries.ITEM.get(ResourceLocation.parse(s));
                    if (itemGet != Items.AIR) {
                        putItem(items, results, itemGet);
                    }
                } else {
                    s = s.substring(1);
                    ResourceLocation tagId = ResourceLocation.parse(s);
