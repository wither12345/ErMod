...
public enum HypostasisCubeState {
    ...
    public void render(ModelPart cube, PoseStack poseStack, VertexConsumer vertexConsumer, int light, int overlay, int tick, float dt){
        ...
        for(CubeState state: states.values()){
            if(state.shouldRender(tick, dt)) {
                poseStack.pushPose();
                poseStack.mulPose(state.getByTime(tick, dt));
    ...
    public record Turning(HypostasisCubeState post, int usingTick){
        public static final StreamCodec<ByteBuf, Optional<Turning>> OPTIONAL_STREAM_CODEC = new StreamCodec<>() {
            public void encode(@NotNull ByteBuf byteBuf, Optional<HypostasisCubeState.Turning> turning) {
            ...
            public @NotNull Optional<HypostasisCubeState.Turning> decode(@NotNull ByteBuf byteBuf) {
                int i = byteBuf.readInt();
                return i == -1 ? Optional.empty() : Optional.of(new HypostasisCubeState.Turning(
