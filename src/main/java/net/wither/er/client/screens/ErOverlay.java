...
public class ErOverlay {
    private static final ResourceLocation STAMINA_SPRITE = new ResourceLocation("er:textures/gui/stamina_full.png");
    private static final ResourceLocation STAMINA_EMPTY_SPRITE = new ResourceLocation("er:hud/stamina_empty.png");
    ...
    private static final ResourceLocation HEALTH_BAR_BASE = new ResourceLocation("er:textures/gui/health/player_health_bar_base.png");
    private static final ResourceLocation HEALTH_BAR_FILLING = new ResourceLocation("er:textures/gui/health/player_health_bar_filling.png");
    private static final ResourceLocation HEALTH_BAR_BLINKING = new ResourceLocation("er:textures/gui/health/player_health_bar_blinking.png");
    private static final ResourceLocation HEALTH_BAR_ABSORBING = new ResourceLocation("er:textures/gui/health/player_health_bar_absorbing.png");
    ...
    public static void onRenderer(RenderGuiOverlayEvent event){
        ...
        if(event.getOverlay() == VanillaGuiOverlay.PLAYER_HEALTH.type()) {
            ...
            renderPlayerArmor(player, event.getGuiGraphics());
            event.setCanceled(true);
        }
    }

	private static void maybeRenderStellaFortunaIcon(@NotNull Player player, GuiGraphics graphics, int w, int h) {
		player.getCapability(ErItemVariables.PLAYER_VARIABLES).ifPresent(
			(var) -> {
				if (var.Stella_Fortuna.getItem() instanceof StellaFortunas fortuna) {
					...
					renderElementalSkillIcon(graphics, w, h, vars);
	...
	private static void renderElementalSkillIcon(GuiGraphics graphics, int w, int h, ErCombatVariables.PlayerVariables vars) {
	...
	private static void maybeRenderPlayerStamina(Player player, GuiGraphics graphics) {
        ...
        if (player != null && minecraft.gameMode != null && minecraft.gameMode.canHurtPlayer() && minecraft.gui instanceof ForgeGui gui) {
            int i1 = graphics.guiWidth() / 2 + 91;
            minecraft.getProfiler().push("stamina");
            double maxStamina = player.getAttributeValue(ErModAttributes.MAX_STAMINA.get());
            ...
            if (j3 < i3) {
                ...
                for (int j4 = 0; j4 < l3 + i4; j4++) {
                    graphics.blit(STAMINA_SPRITE, i1 - j4 * 8 - 9, j2, 0, 0, 9, 9, 9, 9);
	...
	private static void renderPlayerHealthBar(Player player, GuiGraphics graphics) {
		Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.gui instanceof ForgeGui gui && minecraft.gameMode != null && minecraft.gameMode.canHurtPlayer() && player != null) {
            int x = graphics.guiWidth() / 2 - 91;
            minecraft.getProfiler().push("er_health");
            float health = player.getHealth();
            int leftHeight = gui.leftHeight;
            float max_health = player.getMaxHealth();
            int y = graphics.guiHeight() - leftHeight;
            RenderSystem.enableBlend();

            int maxWidth = 81;
            int height = 9;

            graphics.blit(HEALTH_BAR_BASE, x, y, 0, 0, maxWidth, height, maxWidth, height);

            graphics.blit(HEALTH_BAR_BLINKING, x, y, 0, 0, calculateHealth(lastHealth, max_health), height, maxWidth, height);
            graphics.blit(HEALTH_BAR_FILLING, x, y, 0, 0, calculateHealth(health, max_health), height, maxWidth, height);

            int absorbPixels = calculateHealth(player.getAbsorptionAmount(), max_health);
            if (absorbPixels > 0) {
                graphics.blit(HEALTH_BAR_ABSORBING, x, y, 0, 0, absorbPixels, height, maxWidth, height);
            }


            Font font = minecraft.font;
            String s = (int) (health + player.getAbsorptionAmount()) + "/" + (int) max_health;
            int j = x - font.width(s) / 2 + 41;
            graphics.drawString(font, s, j, y + 1, 0xffffff, false);

            if (minecraft.level != null && lastDamageTick + 20 >= minecraft.level.getGameTime()) {
                j = x + font.width(s) / 2 + 45;
                graphics.drawString(font, "-" + lastDamage, j + 1, y + 1, 0, false);
                graphics.drawString(font, "-" + lastDamage, j, y + 2, 0, false);
                graphics.drawString(font, "-" + lastDamage, j - 1, y + 1, 0, false);
                graphics.drawString(font, "-" + lastDamage, j, y, 0, false);
                graphics.drawString(font, "-" + lastDamage, j, y + 1, (minecraft.level.getGameTime() % 8 >= 4 ? damageColor : 0xffffffff), false);
            }
	...
	private static void renderPlayerArmor(Player player, GuiGraphics graphics) {
		Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.gameMode != null && minecraft.gui instanceof ForgeGui gui && player != null && minecraft.gameMode.canHurtPlayer()) {
            ...
            if (armor > 0) {
                ...
                graphics.blit(GUI_ICONS_LOCATION, l, j, 34, 9, 9, 9);
                Font font = minecraft.font;
	...
	public static void updateDamage(int damage, int color, Minecraft minecraft){
		lastDamage = damage;
		damageColor = color ;
		lastDamageTick = minecraft.level == null ? 0 : minecraft.level.getGameTime() ;
	}
}