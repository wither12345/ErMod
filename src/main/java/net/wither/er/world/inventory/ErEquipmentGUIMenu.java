...
public class ErEquipmentGUIMenu extends AbstractContainerMenu{
    ...
    private static class VisionSlot extends ItemTagSlot{
        private static final TagKey<Item> VISION_TAG = ItemTags.create(ResourceLocation.parse("er:vision"));
        private final Player player;

        public VisionSlot(Player player, Container container, int id, int x, int y) {
            super(VISION_TAG, container, id, x, y);
            this.container.setItem(this.getSlotIndex(), player.getData(ErItemVariables.PLAYER_VARIABLES).Vision.copy());
            this.player = player;
        }
    ...
    private static class StellaFortunaSlot extends ItemTagSlot{
        private static final TagKey<Item> STELLA_FORTUNA_TAG = ItemTags.create(ResourceLocation.parse("er:stella_fortuna"));
        private final Player player;

        public StellaFortunaSlot(Player player, Container container, int id, int x, int y) {
            super(STELLA_FORTUNA_TAG, container, id, x, y);
            this.container.setItem(this.getSlotIndex(), player.getData(ErItemVariables.PLAYER_VARIABLES).Stella_Fortuna.copy());
            this.player = player;
        }
    ...
    private static class ArtifactItemSlot extends Slot{
        ...
        public ArtifactItemSlot(ErEntityInterface player, ArtifactSlot slot, Container container, int id, int x, int y) {
            ...
            this.empty = ResourceLocation.fromNamespaceAndPath(ErMod.MODID, "item/" + slot.getSerializedName() + "_empty");
        }

        @Override
        public boolean mayPlace(@NotNull ItemStack itemStack) {
            ArtifactData data = itemStack.get(DataComponentsRegister.ARTIFACT);
