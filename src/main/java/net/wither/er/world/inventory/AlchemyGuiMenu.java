...
public class AlchemyGuiMenu extends AbstractContainerMenu {
    ...
    private ItemStack getCraftingResult(Level world) {
        ...
        Stream<AlchemyCraftingRecipe> recipes = rm.getAllRecipesFor(AlchemyCraftingRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).filter(this::test);
    ...
    public enum Stage{
        ...
        public static StreamCodec<ByteBuf, Stage> STREAM_CODEC =
                StreamCodec.ofMember(Stage::encode, Stage::getByBuffer);

        public static Stage getByBuffer(ByteBuf buffer){
