...
public class ArcEntity extends Entity {
		...
		if (restTime == 1) {
			...
			if (entity != null && entity.isAlive()) {
				...
				if (this.sourceEntity != null) {
					elemental_mastery = this.sourceEntity.getAttribute(ErModAttributes.ELEMENTAL_MASTERY).getValue();
					level = EntityHurtEvent.getEntityLevel(this.sourceEntity);
				}
				entity.hurt(new ErDamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("er:reaction"))), this.sourceEntity, 4, 0),
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		builder.define(DATA_ID_TARGET, 0);
	}
