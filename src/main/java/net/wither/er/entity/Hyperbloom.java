...
public class Hyperbloom extends Projectile {
    ...
    public void tick() {
        ...
        if (target != null) {
            Vec3 delta = target.getPosition(target.getBbHeight() / 2f).subtract(this.getPosition(0)).normalize().multiply(moving_speed, moving_speed, moving_speed) ;
            this.push(delta.x,delta.y,delta.z);
        }
