...
public class PyroSlime extends ElementalSlime{
    ...
    private void explode(){
        ...
        if (this.level() instanceof Level _level) {
            if (!_level.isClientSide()) {
                _level.playSound(null, this.getOnPos(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("entity.generic.explode")), SoundSource.NEUTRAL, 1, 1);
            } else {
                _level.playLocalSound(this.getX(), this.getY(), this.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("entity.generic.explode")), SoundSource.NEUTRAL, 1, 1, false);
            }
        }
            ...
            if (EntityHurtEvent.shouldHurt(this, iterator)) {
                iterator.hurt(
                        ElementSource.createDamageSource(
                                this.level().holderOrThrow(DamageTypes.MOB_ATTACK),
                                this,
                                new ElementSource(ElementRegistry.PYRO.get(), ResourceLocation.parse("er.slime.explode"), 2, true)
