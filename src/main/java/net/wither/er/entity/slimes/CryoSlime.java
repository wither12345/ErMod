        ...
        for(int dx = -2 ; dx <= 2 ; dx ++)
            for(int dy = -1; dy <= 1 ; dy++)
                for(int dz = -2; dz <= 2 ; dz++)
                    if(this.level().getBlockState(pos.offset(dx,dy,dz)).getBlock() == Blocks.WATER && this.level().getBlockState(pos.offset(dx,dy + 1,dz)).is(BlockTags.create(ResourceLocation.parse("minecraft:air")))){
