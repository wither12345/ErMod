...
public class ${name}Entity extends ${extendsClass} <#if interfaces?size gt 0>implements ${interfaces?join(",")}</#if> {
	...
	private class VibrationUser implements VibrationSystem.User {
		...
		@Override public void onReceiveVibration(ServerLevel world, BlockPos vibrationPos, GameEvent holder, @Nullable Entity vibrationSource, @Nullable Entity projectileShooter, float distance) {
